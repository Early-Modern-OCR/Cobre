...
from libros.bookreader.models import Page
...
def bitstream_metadata(book):
    ...
    for annotation in book.annotations.order_by('offset'):
        ...
        annotationElem = etree.Element('annotation',
					...
					'isTypeface': str(annotation.typeface)})
    ...
    for page in book.pages.exclude(title__in=_exterior_views).order_by('sequence'):
        ...
        if not page.jp2:
            ...
            e.extend(create_handles(page))
            for tr in page.transcriptions.filter(vetted = True):
                e.extend(create_transcription(tr))
...
def create_transcription(tr):
    h = [etree.Element('transcription', {'id':str(tr.pk),'author':str(tr.author.username) })]
    h[0].text = tr.text
