...
class BookHarvester(object):
    ...
    def update_metadata(self, metadataPrefix='dim'):
        ...
        """book.contributor = item['contributor']"""
    ...
    def create_pages(self):
        ...
        #This sort is very fragile and was designed very specifically for the Primeros Libros filenames.
        ...
        for page in pages:
            ...
            Page.objects.create(book=self.book, jp2=page['url'].rsplit('?',1)[0].strip(),
    ...
    def update_structure(self, parser=etree.parse):
        ...
        if not self.book.type == 'work':        
            ...
            self.create_transcriptions(document)
    ...
    def update_interior(self, document):
        ...
        for chunk in document.iterfind('.//chunk'):
            try:
                ...
                print "Chunk has jp2 of " + jp2.text.strip()
                ...
                print "Getting or creating page..."
    ...
    def create_transcriptions(self, document):
        """ Re-create all transcriptions from bitstream metadata bitstream """
        log.info('Create transcriptions from the bitstream metadata bitstream')
      
          
          #pages = Page.objects.filter(book=self.book)
          #        trans = Transcription.objects.get(page in pages)
          
        trans = Transcription.objects.filter(page__book=self.book)
      
        log.info('Removing %d transcriptions from %s' % (trans.count(),
                                                      str(self.book.identifier),))
        
        trans.delete();
          #        Transcription.objects.filter(book=self.book).delete()
      
      # Iterate over the chunks and scan for transcriptions
      
        sequence = 1

        for chunk in document.iterfind('.//chunk'):
          try:
            transcription = chunk.find('transcription')
            jp2 = chunk.find('handle[@mimetype="image/jp2"]')
          
            if transcription is not None:
              try:
                jp2 = jp2.text.strip()
                page = Page.objects.get(book=self.book,jp2=jp2)
                authorName = transcription.get('author',None);
                
                try:
                    author = User.objects.get(username=authorName)
                except:
                    # Seems like bad mojo. She should probably make sure that users exist ahead of time
                    author = User.objects.create_user(authorName,authorName,'')
                
                Transcription.objects.create(page=page,
                                             vetted=True,
                                             author=author,
                                             text=transcription.text)
              except:
                log.exception('Error creating transcription #%d' % (sequence,))

          except:
            log.exception('Error interating over transcriptions #%d' % (sequence,))
          
          sequence += 1
        
        log.info('Created %d transcriptions for %s' % (Transcription.objects.filter(page__book=self.book).count(),
                                                    str(self.book.identifier),))
