...
from libros.bookreader.forms import PageForm, ReferenceForm, AnnotationForm, BookForm
from libros.bookreader.models import Collection, Book, Page, Annotation
from libros.bookreader.views.book import view as book_view
from libros.bookreader.export import bitstream_metadata
...
def update_page_order(request, object_id, **kwargs):
    ...
    try:
        ...
        if request.is_ajax():
            """ this error was getting returned spuriously """
            """ return ajax(request,'{"error": "Invalid page supplied"}') """
            return ajax(request, {"success": True})
    ...
    if len(pages) != book.pages.filter(internal=True).count():
        if request.is_ajax():
            """ this error was being returned spuriously """
        ...
        '''<mdWrap MDTYPE="OTHER" OTHERMDTYPE="DIM">
             <xmlData>
                 <dim:dim xmlns:dim="http://www.dspace.org/xmlns/dspace/dim">
                     ...
                     <dim:field mdschema="dc" element="type">%s</dim:field>
                 </dim:dim>
             </xmlData>
         </mdWrap>''' % (book.title, book.type))
