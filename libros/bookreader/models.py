...
from libros.bookreader.fields import MultiValueDictField
    ...
    def clean_fields(self, exclude=None):
        ...
        if not 'handle' in exclude:
            if not self.repository.validate_collection_handle(self.handle):
                message = '%s is not a valid collection handle for %s with exclusions %s' % (self.handle,
                                                                          str(self.repository), str(exclude))
...
class Book(models.Model):
    ...
    def get_extant_manifestations(self):
        return self.manifestations.filter(type='extant')
    
    def get_frankenbook_manifestations(self):
        return self.manifestations.filter(type='frankenbook')
...
class Page(models.Model):
    ...
    def transcriptions(self):
    	if not self.internal:
            return []
        return list(Transcription.objects.raw("""
            SELECT
                *
            FROM
                bookreader_transcription
            WHERE
                bookreader_transcription.page_id = %s""",
            [self.pk]))
...
class Annotation(models.Model):
    ...
    local = models.BooleanField(default=False, blank=False)
    typeface = models.BooleanField(default=False, blank=False)
...
class Transcription(models.Model):
    page = models.ForeignKey(Page, related_name='transcriptions')
    text = models.TextField()
    vetted = models.BooleanField(default=False, blank=False)
